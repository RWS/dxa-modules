...
<jsp:useBean id="entity" type="com.sdl.dxa.modules.core.model.entity.Place" scope="request"/>
<jsp:useBean id="markup" type="com.sdl.webapp.common.markup.Markup" scope="request"/>
<jsp:useBean id="screenWidth" type="com.sdl.webapp.common.api.ScreenWidth" scope="request"/>
<c:set var="width" value="${not empty entity.location ? 6 : 12}"/>
<c:set var="daddr"
       value="${not empty entity.location ? (not empty entity.location.query ? entity.location.query : (entity.location.latitude + ',' + entity.location.longitude)) : ''}"/>
                    ...
                    <div class="vcard">
                        <c:if test="${not empty entity.address}">
                            ...
                            <c:if test="${not empty daddr}">
                                <a class="popup-iframe popup-mobile-ignore"
                                   href="//maps.google.com/maps?saddr=My+Location&amp;daddr=${daddr}">${markup.resource("core.directionsLinkText")}</a><br/>
                                <a class="popup-iframe popup-mobile-ignore"
                                   href="//maps.google.com/maps?q=${daddr}">${markup.resource("core.largeMapLinkText")}</a>
                            </c:if>
                        </c:if>
                        <c:if test="${not empty entity.telephone or not empty entity.faxNumber or not empty entity.email}">
                            ...
                            <div class="h-card">
                                <c:if test="${not empty entity.telephone}">
                                    <p>${markup.resource("core.teleponeCaptionText")} <a class="tel"
                                                                                         href="tel:${entity.telephone}" ${markup.property(entity, "telephone")}>${entity.telephone}</a>
                                    </p>
                                </c:if>
                                <c:if test="${not empty entity.telephone}">
                                    <p>${markup.resource("core.faxCaptionText")} <a class="fax"
                                                                                    href="fax:${entity.faxNumber}" ${markup.property(entity, "faxNumber")}>${entity.faxNumber}</a>
                                    </p>
                                </c:if>
                                <c:if test="${not empty entity.telephone}">
                                    <p>${markup.resource("core.emailCaptionText")} <a class="email"
                                                                                      href="mailto:${entity.email}" ${markup.property(entity, "email")}>${entity.email}</a>
                                    </p>
                                </c:if>
                            </div>
                        </c:if>
