...
if (!($pscmdlet.ShouldProcess($cmsUrl, "Import DXA Google Analytics Module"))) { return }
...
function Add-ComponentToPage($sitePublication, $rootStructureGroup, $componentPath, $templatePath, $pagePath) {
    $pageWebdavUrl = "/webdav/$sitePublication/$rootStructureGroup/$pagePath.tpg"
    $componentWebdavUrl = "/webdav/$sitePublication/$rootFolder/$componentPath.xml"
    $templateWebdavUrl = "/webdav/$sitePublication/$rootFolder/$templatePath.tctcmp"
    ...
    $component = $coreServiceClient.Read($componentWebdavUrl, $defaultReadOptions)
    $template = $coreServiceClient.Read($templateWebdavUrl, $defaultReadOptions)
    $notAdded = $true
    ...
    if ($notAdded)
    {
        ...
        $cps += $cp
        $page = $coreServiceClient.CheckOut($page.Id, $false, $defaultReadOptions)
        $page.ComponentPresentations = $cps
        $page = $coreServiceClient.Save($page, $defaultReadOptions)
        $page = $coreServiceClient.CheckIn($page.Id, $true, $null, $defaultReadOptions)
        Write-Output "Done"
    }
    else
    {
...
if ($importType -ne "permissions-only")
{
    ...
    else
    {
    	Add-ComponentPresentation `
            "/webdav/$sitePublication/$rootFolder/Settings/GoogleAnalytics/Site Manager/Google Analytics Configuration.xml" `
            "/webdav/$sitePublication/$rootFolder/Modules/GoogleAnalytics/Site Manager/Templates/Google Analytics.tctcmp" `
            "/webdav/$sitePublication/$rootStructureGroup/_System/include/Footer.tpg" `
            3 `
    		"Links"
    }
}
