...
if (!($pscmdlet.ShouldProcess($cmsUrl, "Import DXA Audience Manager Module"))) { return }
...
$importPackageFullPath = Join-Path $distSource "module-AudienceManager.zip"
...
if ($importType -ne "permissions-only")
{
    ...
    if(Is-Web8) 
    {
        Add-ComponentPresentation-Web8 `
        "$siteRootFolderWebDavUrl/Modules Content/AudienceManager/Current User Widget.xml" `
        "$siteRootFolderWebDavUrl/Modules/AudienceManager/Site Manager/Templates/Current User Widget [Nav].tctcmp" `
    ...
    else
    {
        Add-ComponentPresentation `
            "$siteRootFolderWebDavUrl/Modules Content/AudienceManager/Current User Widget.xml" `
            "$siteRootFolderWebDavUrl/Modules/AudienceManager/Site Manager/Templates/Current User Widget.tctcmp" `
            "$siteRootSgWebDavUrl/_System/include/Header.tpg" `
            2 `
            "Nav"
    }
